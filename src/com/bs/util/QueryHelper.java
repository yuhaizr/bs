...
public class QueryHelper {
	...
	public QueryHelper(Class<?> clazz, String alias) {
		fromClause = new StringBuffer(" FROM " + clazz.getSimpleName() + " "
				+ alias);
	}
	...
	public QueryHelper addWhereCondition(boolean append, String condition,
			Object... param) {
		if (append) {
	...
	public QueryHelper addOrderByProperty(String propertyName, boolean asc) {
		if (orderByClause == null) {
			orderByClause = new StringBuffer(" ORDER BY  " + propertyName
					+ (asc ? " ASC " : " DESC "));
		} else {
			orderByClause.append(" , " + propertyName
					+ (asc ? " ASC " : " DESC "));
		}
		return this;
	}
	...
	public String getListQueryHQL() {
		return "" + fromClause + (whereClause == null ? "" : whereClause)
